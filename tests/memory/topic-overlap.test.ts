...
describe("findTopicOverlaps", () => {
  it("finds topic-related fragments in 40-65% similarity range", () => {
    const fragments = [
    ...
    const overlaps = findTopicOverlaps(fragments, "Next.js App Router is the recommended approach", null);
    assert.ok(overlaps.length >= 1, "Should find at least 1 topic overlap");
    assert.ok(overlaps.some(o => o.fragment.includes("Next.js")), "Should match Next.js fragment");
  });

  it("does not return fragments below 40% similarity", () => {
    const fragments = [
      makeFragment("Completely unrelated topic about cooking recipes"),
      makeFragment("Another unrelated topic about gardening"),
    ];

    const overlaps = findTopicOverlaps(fragments, "React hooks state management patterns", null);
    assert.equal(overlaps.length, 0);
  });

  it("returns empty for empty memory", () => {
    const overlaps = findTopicOverlaps([], "React hooks", null);
    assert.equal(overlaps.length, 0);
  });
  ...
  it("respects project scoping", () => {
    ...
    const overlaps = findTopicOverlaps(fragments, "React state patterns", "projectA");
    assert.ok(overlaps.length >= 1);
    assert.ok(overlaps.every(o => o.project === "projectA" || o.project === null));
  });
  ...
  it("respects limit parameter", () => {
    ...
    const overlaps = findTopicOverlaps(fragments, "React component architecture", null, 3);
