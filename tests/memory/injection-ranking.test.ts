...
describe("Injection ranking (confidence x recency)", () => {
  it("prioritizes recent high-confidence over old high-confidence", () => {
    const fragments = [
    ...
    const results = searchAndSortFragments(fragments, null, 10);
    assert.ok(results.length >= 2);
    assert.ok(results[0].fragment.includes("Recent"), `Expected recent first, got: ${results[0].fragment}`);
  });

  it("still surfaces very high confidence even if older", () => {
    const fragments = [
  ...
  it("ranks by query relevance first, then by injection score", () => {
    const fragments = [
    ...
    const results = searchAndSortFragments(fragments, "React hooks", 10);
    assert.ok(results.length >= 1);
    assert.ok(results[0].fragment.includes("React"), "Query-relevant results should come first");
  });

  it("returns all fragments when no query", () => {
    const fragments = [
  ...
  it("respects topK limit", () => {
    ...
    const results = searchAndSortFragments(fragments, null, 5);
